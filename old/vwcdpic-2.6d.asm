...
;;; PIC12F675 support is not tested, but should work.
	IFDEF __12F675
#include <p12f675.inc>
	ENDIF

;;; PIC16F627 support is not implemented, but should be fairly easy to do.
	IFDEF __16F627
#include <p16f627.inc>
	ENDIF
...
; experimental Archos Jukebox 9600 baud remote control support. 
; currently disabled - may be causing bugs with european head units?
;#define ARCHOS_SUPPORT
;--------------------------------------------------------------------------
; PIC12F629 Connections
;--------------------------------------------------------------------------
...
; DISCSTART can be any number less than DISCMIN-1. Difference
...
intwsave		EQU	GPRAM+11
intstatussave	EQU	GPRAM+12
intfsrsave		EQU	GPRAM+13

progflags		EQU	GPRAM+14

; the 'capbusy' flag will be set when the ISR is busy capturing a command packet
; from the head unit. The TMR0 ISR will clear it once the recieve timeout has
; been exceeded or the PWTX capture ISR will clear it once 32 bits have been
; captured (command packets are supposed to be 32 bits long only).
capbusy			EQU	0
...
captime			EQU	GPRAM+15	; timer count of low pulse (temp)
capbit			EQU	GPRAM+16	; bits left to capture for this byte
capbitpacket	EQU	GPRAM+17	; bits left to capture for the entire packet
...
secondcount		EQU	GPRAM+22
scancount		EQU	GPRAM+23
...
;; weak pullups are enabled by default at power on reset.
;;	clrf	WPU
...
;	goto 	SendDisplayPacket 	; force first display update packet
SendDisplayPacket:
...
FirstByteTest:
	...
	movf	scanbyte, w
	call	EnqueueHex
	call	SaveScanPointer		; save new scanptr so we don't look at this byte
	goto	FirstByteLoop
...
FourthByte:
	...
	; if execution reaches here, we have verified that 
	; bytes 1 and 2 are valid for a comment packet, but
...
ButtonMIX:
	...
	subwf	buttonbyte1, w
	btfss	STATUS, Z
	goto	ButtonMIX6		; might still be mix button, check other code
...
;; Hopefully we don't really need to disable interrupts afterall...
...
;; wait 335us for head unit to store sent byte - didn't work so good on 
;; late 2003 wolfsburg double din, so we now wait 700us instead.
;	movlw	-84
...
EnqueueHex:
	movwf	buttonbyte1
	swapf	buttonbyte1, w		; send high nibble first
	andlw	0x0F
	movwf	sendbitcount
	...
	rlf		buttonbyte1, f	; multiply low nibble by 2.
	movf	buttonbyte1, w	; now send low nibble
...
;;; Copyright notice stored in EEPROM data memory.
	...
	ORG 2100h
	de		"VWCDPIC Firmware v", VER_MAJOR, ".", VER_MINOR, "d\n"
