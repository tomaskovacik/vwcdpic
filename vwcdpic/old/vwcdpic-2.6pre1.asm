...
PWTXCaptureBit
	...
	;; yes, we need to reset the capture frame to the beginning of a byte
	incf	FSR, f
	movlw	-8
	movwf	capbit	
...
IdleLoopSkipSend
IF DUMPMODE == 1
	btfss	progflags, capdone ; only use serial port (turns off interrupts) when not capturing
	goto	IdleLoop	; busy capturing, don't mask int

	btfss	progflags, startbit
	goto	TryDump

	bcf		progflags, startbit
	call	SendNEWLINE

TryDump
	call	GetCaptureByte	; see if we can dump data
	btfsc	STATUS, Z
	goto	IdleLoop
	movf	scanbyte, w
	call	SendSerialHex
