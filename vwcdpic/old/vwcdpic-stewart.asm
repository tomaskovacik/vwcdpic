;; Volkswagen CD Changer Emulator and Head Unit Command Sniffer
;; For use on PIC16F627 at 4MHz/5VDC
;;
;; Copyright (c) 2002, Edward Schlunder <zilym@yahoo.com>
;;
;; Monsoon state information contributed by Andy Wilson <awilson@microsoft.com>
;; Monsoon debug work contributed by Paul Stewart <stewart@parc.com>
;;
;; $Log: vwcdpic-stewart.asm,v $
;; Revision 1.1  2003/04/04 07:52:52  edwards
;; Initial import
;;
;; Revision 1.7.2.2.2.1  2002/12/01 09:06:06  edwards
;; Committed Paul Stewart's semi-working Monsoon code to CVS branch.
;;
...
;       So each PIC instruction takes one microsecond long.
	...
	LIST P=16F627, R=DEC
	__CONFIG _BODEN_ON & _DATA_CP_OFF & _LVP_OFF & _MCLRE_OFF & _WDT_OFF & _PWRTE_ON & _CP_OFF & _INTRC_OSC_CLKOUT
...
;Rev 1.1 - removed all instances of ready-modify-write's to PORT regs
;          added CD[1..6] button recognition
;	   made display update send identification strings to serial
;Rev 1.1b - fixed SendSerialHex bugs
;Rev 1.2 - added first pass at a Monsoon state implementation, untested.
;Rev 1.3 - this is a command datastring dumper only
;--------------------------------------------------------------------------
; Variables
;--------------------------------------------------------------------------
...
IdleLoopSkipSend
; Comment out following two lines to enable dumper
	call	ScanButtons
	goto	IdleLoop

; ----------------- SERIAL DUMP CODE START ---------------------
        movf    FSR, w          ; have we caught up with the capturer?
        subwf   capptr, w
        btfsc   STATUS, Z
	goto	EndofLine

        movf    INDF, w         ; send a byte from the capture buffer
        call    SendSerialHex
...
EndofLine
        btfss   progflags, capdone ; are we done with this packet?
        goto    IdleLoop

        bcf     progflags, capdone ; clear done flag
        movlw   13
        call    SendSerial
        movlw   10
        call    SendSerial
...
;
; Monsoon only:
; xxxxB0xxxx enable (transition to State 2)
; xxxxB4xxxx disc loaded inquiry? (transition to State 5)
; xxxxEExxxx disable (transition to State 1)
; 95694EB080 enable (goto state 2)
; 956938C680 disc loaded inquiry (goto state 5)
; 956910EE80 disable (goto state 1)
;--------------------------------------------------------------------------
MonsoonEnable
	movlw	0x4E			; PJS corrected button bytes
	subwf	buttonbyte1, w		; Possible conflict w/non-Monsoon
	btfss	STATUS, Z		; button-release code
	return	
...
MonsoonDisable			; PJS XXX I think the 'EE' should be in 
	movlw	0xEE		; byte 2, but this conflicts ButtonMODECHG
	subwf	buttonbyte1, w	; so I'll just put a conditional there.
	btfss	STATUS, Z
	return	
...
ButtonMODECHG
	...
	subwf	BIDIstate, w
	btfss	STATUS, Z
	goto	MonsoonReset	; PJS We might have been in a Monsoon state
...
;--------------------------------------------------------------------------
SendPacket
; Monsoon protocol has 5 states. Thanks to 
; Andy Wilson <awilson@microsoft.com> for providing 
; information about them.
;
; Non-Monsoon head units will most likely stay in State 1 only.
;
	movlw	HIGH SendPacket
	movwf	PCLATH
	movf	BIDIstate, w
	addwf	PCL, f		;todo: Verify byte alignment such that this
	nop			;doesn't overflow PCLATH
	goto	BIDIstate1	
	goto	BIDIstate2
	goto	BIDIstate3
	goto	BIDIstate4
