...
;; AUDIO CD Load found by Svetoslav Vassilev <svetoslav.vassilev@excite.com>
;;
;; $Log: vwcdpic.asm,v $
;; Revision 1.20  2003/07/07 17:41:52  edwards
;; Changed disc load packets to 0x29..0x2F.
;;
...
Start
	...
	movlw	11000100b		; port b pull-ups disabled
...
LowBit9600
	bsf		STATUS, RP0			; select data bank 1
	ERRORLEVEL -302
	bcf		TRISB, SerialTX9600 ; set SerialTX9600 pin as output/drain (low)
	ERRORLEVEL +302
	bcf		STATUS, RP0			; go back to data bank 0
	call	Wait46
	call	Wait46
...
BitCount9600
	...
	bsf		STATUS, RP0			; select data bank 1
	ERRORLEVEL -302
	bsf		TRISB, SerialTX9600 ; set SerialTX9600 pin as input/open (high)
	ERRORLEVEL +302
	bcf		STATUS, RP0			; go back to data bank 0
	call	Wait46
	call	Wait46
