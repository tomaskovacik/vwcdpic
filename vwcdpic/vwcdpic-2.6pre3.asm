...
; Precompiler Options
ARCHOS_SERIAL	EQU 0	; send 9600 baud Archos Jukebox remote control codes
DUMPMODE		EQU 0	; dump all command data packets to serial port, doesn't
						; try to identify them at all. useful for debugging
						; protocol only.
...
txinptr			EQU	GPRAM+25
txoutptr		EQU	GPRAM+26
txbuffer		EQU	GPRAM+27
txbufferend		EQU	GPRAM+46	; 39-27-1 = 11 serial output strings queue

capbuffer		EQU	GPRAM+39	; 64-39 = 25 bytes for head unit command
...
Start
	...
	movlw	txbuffer			; initialize empty outgoing serial string
	movwf	txinptr				; queue
	movwf	txoutptr
	...
	call	EnqueueStringIDENTIFYNEWLINE
	movlw	low sRING
	call	EnqueueString
	movlw	low sNEWLINE
	call	EnqueueString
...
SendTXStringByte
	...
	call	LoadSerial			; put w into serial sendreg and init sendbitcount
	...
	goto	DoSend				; if we're not capturing, we can send serial

	movlw	32					; if we've got 32 bits since last start
	subwf	capbittotal, w		; bit, we're probably okay to send serial data
	btfsc	STATUS, C
	goto	DoSend

	;; it may not be safe to mask interrupts right now, so abort sending
	;; this byte until later when it becomes safe.
	goto	IdleLoop

DoSend:
	call	SendSerialSkipSpin
...
GetNextStringByte:
	...
	movf	INDF, w
	movwf	PCL
	;; execution should never reach here, addwf above jumps to
	;; SerialDataStrings jump table and retlw's value.
...
FirstByteTest
	...
	movf	scanbyte, w
	call	SendSerialHex
	movf	FSR, w				; save new scanptr so we don't look at this byte
	movwf	scanptr				; again.
...
;--------------------------------------------------------------------------
SendSerial
	call	LoadSerial			; save w into sendreg and init sendbitcount
	call	ReceiveClearSpin	; make sure we've okay to mask interrupts
...
;; --------------------------------------------------------------
;; Serial Strings. Must start on 256 byte page boundary for this
;; to work.
;;	ORG		768
SerialDataStrings
sDATAERR
...
;;; Copyright notice stored in EEPROM data memory.
	...
	de		"Copyright (c) 2003-2004, Edward Schlunder <ed@k9spud.com>\n"
