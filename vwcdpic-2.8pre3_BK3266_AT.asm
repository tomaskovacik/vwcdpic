...
;; $Log: vwcdpic.asm,v $
;; 18.11.2021 - BT4
;; kovo = add support for binary HY5066SR protocol .... 
;; using #DEFINE HY5066SR
;;
;; 2021 - BT3
;; kovo = one way(no feedback) BT support for uart controled modules like BK8000L,bk3254,bk3266 - atversions, no bin versions
;;
...
EndString:
	...
	IFDEF IPOD_SUPPORT
EnqueueIPODPREAMBLE:		
		movlw	low iipodPREAMBLEpodPREAMBLE
		goto	EnqueueString
	ENDIF
...
;;binary protocol for HY5066SR
;;
;; CRC = SUM & 0xFF
;;
;; send start: 0x52 0x42 0x23 
;; send end: 0x23 0x42 0x52
;;
;; enable receiver:
;; start + 0x04 + 0x01 + 0x25 + 0x2A + end
;;
;; play/pause
;; start + 0x04 + 0x01 + 0x0B + 0x10 + end
;;
;; stop
;; start + 0x04 + 0x01 + 0x0C + 0x11 + end
;;
;; next
;; start + 0x04 + 0x01 + 0x0D + 0x12 + end
;;
;; previous
;; start + 0x04 + 0x01 + 0x0E + 0x13 + end
;;
;;
...
EnqueueHYPACKETSTART:
                movlw   low sHYPACKERSTART
		call    EnqueueString
		return

EnqueueHYPACKETSTOP:
		movlw	low sHYPACKERSTOP
		call    EnqueueString
		return

EnqueueBTPLAYPAUSE:
                call    EnqueueHYPACKETSTART
                movlw   low sHYPLAYPAUSE
                call    EnqueueString
                call    EnqueueHYPACKETSTOP
                goto    EnqueueNEWLINE
...
IFDEF HY5066SR
sHYPACKETSTART:
	dt	0x52,0x42,0x23,0x04,0x01 ;;we are sending only 4bytes for all commands, so we can have size of the packet here also the command type (0x01) is always same
	goto	EndStrig
sHYPACKETSTOP:
	dt	0x23,0x42,0x52
	goto	EndString
